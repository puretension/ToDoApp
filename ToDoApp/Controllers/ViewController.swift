...
extension ViewController: UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell.memoData = toDoData[indexPath.row]
...
extension ViewController: UITableViewDelegate {
    //지금까지의 구현 방식(셀 자체 클릭. didSelectRowAt과 performSegue)
...
//    // (세그웨이를 실행할때) 실제 데이터 전달 (ToDoData전달)
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "ToDoCell" {
            ...
            detailVC.MemoData = toDoManager.getToDoListFromCoreData()[indexPath.row]
        }
    }
    // new 구현방식(셀(터치X) 위에 올라가 있는 버튼 터치시 다음화면으로 이동함
