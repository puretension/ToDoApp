...
final class CoreDataManager {
    ...
    func getToDoListFromCoreData() -> [MemoData] {
        ...
        if let context = context {
            //NSFetchRequest<NSManagedObject>(entityName: <#T##String#>)
            // 요청서
    ...
    func saveMemoData(toDoText: String?, colorInt: Int64, completion: @escaping () -> Void) {
        // 임시저장소 있는지 확인
        if let context = context {
            ...
            if let entity = NSEntityDescription.entity(forEntityName: self.modelName, in: context) {
                ...
                if let MemoData = NSManagedObject(entity: entity, insertInto: context) as? MemoData {
                    ...
                    // MARK: - MemoData에 실제 데이터 할당 ⭐️
                    MemoData.memoText = toDoText
